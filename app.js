...
let globalStages = [];
let emailTemplates = [];
let currentUser = ""; let currentRole = ""; let currentName = ""; let currentPic = ""; let currentEmail = ""; let currentUserIdPhone = "";
let currentUserIdDb = null;
let currentUserIP = "";
let currentModalMode = 'add'; let activeRowIndex = null;
let currentClientPage = 1; const rowsPerPage = 100;
let stageChartInstance = null; let workloadChartInstance = null;
...
function whatsappCall(phone) {
    ...
    if(num.length >= 10) {
        ...
        showToast("User ID theke number pawa jayni", "error");
    }
}
...
async function saveEmailTemplate() {
    ...
    if(activeEmailTempId) {
        const { error } = await _supabase.from('emailtemp').update({ emailname: name, emailsubject: sub, emailbody: body, createdby: currentUserIdDb }).eq('emailtempid', activeEmailTempId);
        resErr = error;
    } else {
...
// --- PWA LOGIC ---
function generateAppLogo() {
    let canvas = document.createElement('canvas');
    canvas.width = 512; canvas.height = 512;
    let ctx = canvas.getContext('2d');
    ctx.fillStyle = '#0077b5';
    ctx.beginPath(); ctx.roundRect(0, 0, 512, 512, 100); ctx.fill();
    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 200px Poppins, sans-serif';
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText('CMT', 256, 256);
    return canvas.toDataURL('image/png');
}

const swCode = `
  const CACHE_NAME = 'cmt-erp-v4';
...
// --- STAGE MANAGE FUNCTIONS (Updated & Professional) ---
...
async function addNewStage() {
    ...
    if (error) {
        ...
        if (typeof fetchStages === 'function') {
            await fetchStages();
            renderStageListInModal();
            renderStageDropdown(); 
        }
...
function openProfileModal() { 
    document.getElementById('p-user').value = currentUser; document.getElementById('p-name').value = currentName; document.getElementById('p-email').value = currentEmail;
    let p = ""; try{ p = JSON.parse(localStorage.getItem('cmt_session') || sessionStorage.getItem('cmt_session')).password; }catch(e){} 
    document.getElementById('p-pass').value = p; document.getElementById('p-uploadText').innerText = "Click to Update Picture"; 
    pUploadedBase64 = ""; pUploadedFileName = ""; pUploadedMimeType = ""; 
    document.getElementById('profileModal').style.display = 'flex'; 
}
function handleProfilePic(e) { let file = e.target.files[0]; if(!file) return; let reader = new FileReader(); reader.onload = function(evt) { let img = new Image(); img.onload = function() { let canvas = document.createElement('canvas'); let ctx = canvas.getContext('2d'); let maxW = 300, maxH = 300; let w = img.width, h = img.height; if(w > maxW) { h *= maxW/w; w = maxW; } if(h > maxH) { w *= maxH/h; h = maxH; } canvas.width = w; canvas.height = h; ctx.drawImage(img, 0, 0, w, h); let dataUrl = canvas.toDataURL('image/jpeg', 0.8); pUploadedBase64 = dataUrl.split(',')[1]; pUploadedFileName = file.name; pUploadedMimeType = 'image/jpeg'; document.getElementById('p-uploadText').innerHTML = `<b style='color:#10b981;'>${file.name} Ready!</b>`; }; img.src = evt.target.result; }; reader.readAsDataURL(file); }

async function saveProfile() { 
    let pName = document.getElementById('p-name').value; let pPass = document.getElementById('p-pass').value; let pEmail = document.getElementById('p-email').value;
  ...
  if(pPass.length < 6) return showToast("Password must be at least 6 characters.", "error");
    ...
    if(!error) { 
        showToast("Profile Updated Successfully!"); addNotification('Profile', 'Profile updated successfully', 'fa-user-edit', '#10b981'); 
        currentName = pName; currentEmail = pEmail;
        document.getElementById('displayUserName').innerHTML = `${getGreeting()}, <b>${currentName}</b>`; 
        if(updateData.profilepic) { currentPic = updateData.profilepic; document.getElementById('avatarImage').src = currentPic; document.getElementById('avatarImage').style.display = 'flex'; document.getElementById('avatarLetter').style.display = 'none'; } 
        let sData = {username: currentUser, password: pPass, role: currentRole, name: currentName, dbId: currentUserIdDb}; 
        if(localStorage.getItem('cmt_session')) localStorage.setItem('cmt_session', JSON.stringify(sData)); else sessionStorage.setItem('cmt_session', JSON.stringify(sData)); 
        document.getElementById('profileModal').style.display = 'none'; 
    } else { showToast("Failed to update profile", "error"); } 
}

const animateNum = (id, endVal) => { let el = document.getElementById(id); let startVal = 0; let duration = 1000; let startTime = null; const step = (timestamp) => { if (!startTime) startTime = timestamp; let progress = Math.min((timestamp - startTime) / duration, 1); el.innerText = Math.floor(progress * endVal); if (progress < 1) { window.requestAnimationFrame(step); } }; window.requestAnimationFrame(step); };
...
function renderAdminDashboard() { 
    ...
    animateNum('dashTotalClients', activeC + inactiveC); animateNum('dashActiveClients', activeC); animateNum('dashInactiveClients', inactiveC); animateNum('dashTotalUsers', totalUsers); 
    let textColor = getComputedStyle(document.documentElement).getPropertyValue('--text-main').trim() || '#333'; let chartLabels = Object.keys(stageCounts).map(s => `${s} (${stageCounts[s]})`); 
    if (stageChartInstance) stageChartInstance.destroy(); let stageCtx = document.getElementById('stageChart').getContext('2d'); 
    ...
    if (workloadChartInstance) workloadChartInstance.destroy(); let workCtx = document.getElementById('workloadChart').getContext('2d'); workloadChartInstance = new Chart(workCtx, { type: 'bar', data: { labels: Object.keys(assignCounts), datasets: [{ label: 'Clients Assigned', data: Object.values(assignCounts), backgroundColor: '#3b82f6', borderRadius: 6 }] }, options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, grid:{color:'rgba(148, 163, 184, 0.1)'}, ticks:{color:textColor, font:{family:'Poppins'}} }, x: { grid:{display:false}, ticks:{color:textColor, font:{family:'Poppins'}} } }, plugins: { legend: { display: false } } } }); 
}

function populateReportDropdowns() { 
    let names = new Set(), stages = new Set(), ids = new Set(), assigns = new Set(); 
    ...
    document.getElementById('repFilterName').innerHTML = nameHtml; document.getElementById('repFilterStage').innerHTML = stageHtml; document.getElementById('repFilterAssigned').innerHTML = assignHtml; 
}
...
function openLaunchpad(index) { 
    ...
    document.getElementById('l-uid').innerText = String(r.user_id || 'N/A').replace(/^'/, ''); document.getElementById('l-pass').innerText = String(r.password || 'N/A').replace(/^'/, ''); 
    document.getElementById('l-btn').onclick = function() { if(url) window.open(url, '_blank'); else showToast('No valid URL.', 'warning'); }; document.getElementById('launchModal').style.display = 'flex'; 
}

function sortTable(n) { /* Sorting logic intentionally omitted to preserve exact structure limit */ }

function toggleSelectAll() { let isChecked = document.getElementById('selectAllCb').checked; let checkboxes = document.querySelectorAll('.row-checkbox'); checkboxes.forEach(cb => cb.checked = isChecked); toggleMultiDeleteBtn(); }
function checkSelected() { let total = document.querySelectorAll('.row-checkbox').length; let checked = document.querySelectorAll('.row-checkbox:checked').length; document.getElementById('selectAllCb').checked = (total === checked && total > 0); toggleMultiDeleteBtn(); }
function toggleMultiDeleteBtn() { let checked = document.querySelectorAll('.row-checkbox:checked').length; if(checked > 0) { document.getElementById('multiDeleteBtn').style.display = 'flex'; document.getElementById('multiDeleteBtn').innerHTML = `<i class="fa fa-trash"></i> Del (${checked})`; document.getElementById('bulkActiveBtn').style.display = 'flex'; document.getElementById('bulkInactiveBtn').style.display = 'flex'; } else { document.getElementById('multiDeleteBtn').style.display = 'none'; document.getElementById('bulkActiveBtn').style.display = 'none'; document.getElementById('bulkInactiveBtn').style.display = 'none'; } }
...
function highlightSearch(text, filter) { if(!filter || filter === '') return text; let regex = new RegExp(`(${filter})`, "gi"); return String(text).replace(regex, "<mark style='background:#fde047; padding:0 2px; border-radius:4px; color:#000;'>$1</mark>"); }

function switchClientView(mode) { currentViewMode = mode; document.getElementById('btnTableView').classList.remove('active'); document.getElementById('btnBoardView').classList.remove('active'); document.getElementById('tableViewWrapper').style.display = 'none'; document.getElementById('boardViewWrapper').classList.remove('active'); if(mode === 'table') { document.getElementById('btnTableView').classList.add('active'); document.getElementById('tableViewWrapper').style.display = 'block'; } else { document.getElementById('btnBoardView').classList.add('active'); document.getElementById('boardViewWrapper').classList.add('active'); renderKanban(); } }
...
function allowDrop(ev) { ev.preventDefault(); ev.currentTarget.classList.add('drag-over'); } function dragLeave(ev) { ev.currentTarget.classList.remove('drag-over'); } function drag(ev) { ev.dataTransfer.setData("rowIdx", ev.currentTarget.getAttribute("data-rowidx")); }
...
async function drop(ev, stageName) { 
    ...
    if(row && row.stage !== stageName) { 
        ...
        row.stage = stageName; row.update_date = getTodayStr(); renderKanban(); renderTable(); addNotification('Stage Moved', `${row.client_name} moved to ${stageName}`, 'fa-random', '#3b82f6'); 
...
function switchTab(tabId) {
    ...
    setTimeout(function() {
        try {
            ...
            console.error("Data Load Error (Menu is safe): ", dataError);
        }
    }, 50);
}
...
function openModal(mode, index = null, sheetRowIdx = null) {
    ...
    try {
        ...
        if (mode === 'add') {
            ...
            document.getElementById('m-email').value = (r.client && r.client.email) ? r.client.email : '';
            document.getElementById('m-address').value = (r.client && r.client.address) ? r.client.address : '';
...
function resetReports() { document.getElementById('repFilterAssigned').value = ''; document.getElementById('repFilterName').value = ''; document.getElementById('repFilterStage').value = ''; document.getElementById('repFilterStatus').value = ''; document.getElementById('repStartDate').value = getFirstDayStr(); document.getElementById('repEndDate').value = getTodayStr(); }

function generateReport() { 
    let fAssign = document.getElementById('repFilterAssigned').value; let fName = document.getElementById('repFilterName').value; let fStage = document.getElementById('repFilterStage').value; let fStart = document.getElementById('repStartDate').value; let fEnd = document.getElementById('repEndDate').value; let fStatus = document.getElementById('repFilterStatus').value; 
    let data = globalClientData;
    ...
    html += '</tbody></table>'; 
    if(count === 0) { document.getElementById('reportResultContainer').innerHTML = "<div style='padding:40px; text-align:center;'>No records found for selected filters.</div>"; document.getElementById('printRealTableBody').innerHTML = ""; } 
    else { let header = `<div style="padding:15px; background:rgba(59,130,246,0.1); color:var(--text-title); font-weight:600; border-bottom:2px solid #3b82f6;">Total Matches Found: ${count}</div>`; document.getElementById('reportResultContainer').innerHTML = header + html; document.getElementById('printRealTableBody').setAttribute('data-print', JSON.stringify(printData)); addNotification('Report Ready', `Generated report with ${count} clients.`, 'fa-file-contract', '#3b82f6'); } 
}

function getPrintHTML(isEmail = false) { let printDataStr = document.getElementById('printRealTableBody').getAttribute('data-print'); if(!printDataStr) return ""; let data = JSON.parse(printDataStr); let sDate = document.getElementById('repStartDate').value; let eDate = document.getElementById('repEndDate').value; let rangeText = "All Time"; if(sDate && eDate) rangeText = `${formatDateBD(sDate)} to ${formatDateBD(eDate)}`; else if(sDate) rangeText = `From ${formatDateBD(sDate)}`; else if(eDate) rangeText = `Until ${formatDateBD(eDate)}`; let html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>body { font-family: sans-serif; color: #000; padding:20px; } table { width: 100%; border-collapse: collapse; margin-top: 20px; font-size: 13px; } th, td { border: 1px solid #000; padding: 10px; text-align: left; } th { background-color: #f1f1f1; -webkit-print-color-adjust: exact; color-adjust: exact; font-size:14px;}</style></head><body><div style="text-align:center; padding-bottom:10px; border-bottom:2px solid #000; margin-bottom:20px;"><h1 style="margin:0; font-size:28px;">E-Mahajon CMT</h1><h2 style="margin:5px 0;">CMT Client Report</h2><p style="margin:0; font-size:14px;">Date Range: ${rangeText} | Location: Dhaka, Bangladesh<br>Web: cmt.com.bd | Email: cmt@gmail.com</p></div><table><thead><tr><th>SL No</th><th>Created Date</th><th>Client Name</th><th>Stage</th><th>Remarks</th><th>Client Status</th></tr></thead><tbody>`; data.forEach(r => { html += `<tr><td style="text-align:center;">${r[0]}</td><td>${r[1]}</td><td><b>${r[2]}</b></td><td>${r[3]}</td><td>${r[4]}</td><td>${r[5]}</td></tr>`; }); html += `</tbody></table><div style="display:table; width:100%; margin-top:80px; font-weight:bold; font-size:15px; page-break-inside: avoid;"><div style="display:table-cell; text-align:left; vertical-align:bottom;"><div style="margin-bottom:5px;">${currentName}</div><div style="border-top:1px solid #000; padding-top:5px; width:200px;">Authorized Signature</div></div><div style="display:table-cell; text-align:right; vertical-align:bottom;"><div style="margin-bottom:5px;">&nbsp;</div><div style="border-top:1px solid #000; padding-top:5px; width:200px; display:inline-block; text-align:left;">Receiver's Signature</div></div></div></body></html>`; return html; }
function printReport() { let html = getPrintHTML(); if(html === '') return showToast("Generate report first to print.", "warning"); let printWindow = window.open('', '_blank'); printWindow.document.write(html); printWindow.document.close(); printWindow.focus(); setTimeout(() => { printWindow.print(); printWindow.close(); }, 500); }

function exportExcel() { let data = globalClientData; if(data.length <= 0) return showToast("No data to export", "warning"); let wb = XLSX.utils.book_new(); let ws_data = [["Date Added", "Client Name", "Stage", "URL", "User ID", "Password", "Document", "Assigned", "Status", "Remarks"]]; for(let i=0; i<data.length; i++) { let r = data[i]; ws_data.push([r.created_date, r.client_name, r.stage, r.login_url, r.user_id, r.password, r.document, r.assigned_person, r.status||'Active', r.remarks||'']); } let ws = XLSX.utils.aoa_to_sheet(ws_data); XLSX.utils.book_append_sheet(wb, ws, "Clients"); XLSX.writeFile(wb, "CMT_Client_Backup.xlsx"); addNotification('Backup Downloaded', 'Excel database backup created.', 'fa-file-excel', '#10b981'); }
function exportCSV() { let table = document.querySelector("#reportResultContainer table"); if(!table) return showToast("Please generate a report first.", "warning"); let csv = []; for(let i=0; i<table.rows.length; i++) { let row = [], cols = table.rows[i].querySelectorAll("td, th"); for(let j=0; j<cols.length; j++) { let val = cols[j].querySelector('.pwd-mask') ? cols[j].querySelector('.pwd-mask').getAttribute('data-pwd') : cols[j].innerText; row.push('"' + val.replace(/"/g, '""') + '"'); } csv.push(row.join(",")); } let downloadLink = document.createElement("a"); downloadLink.download = "CMT_Report_" + Date.now() + ".csv"; downloadLink.href = window.URL.createObjectURL(new Blob([csv.join("\n")], {type: "text/csv"})); downloadLink.style.display = "none"; document.body.appendChild(downloadLink); downloadLink.click(); }

function renderTeamActivity() { let data = globalClientData; let counts = {}; let total = data.length; for(let i=0; i<data.length; i++) { let owner = data[i].assigned_person || 'Unassigned'; counts[owner] = (counts[owner] || 0) + 1; } let html = ''; for(let user in counts) { let pct = total > 0 ? Math.round((counts[user] / total) * 100) : 0; html += `<div class="dash-card" style="display: flex; align-items: center; gap: 20px; border-top: 3px solid #3b82f6;"><div style="width: 55px; height: 55px; background: rgba(59,130,246,0.1); color: #3b82f6; border-radius: 50%; display: flex; justify-content: center; align-items: center; font-size: 24px; font-weight: bold; border: 2px solid rgba(59,130,246,0.2);"><i class="fa fa-user-tie"></i></div><div style="flex: 1; z-index:2;"><h4 style="margin: 0 0 8px 0; color: var(--text-title); font-size: 15px; display: flex; justify-content: space-between;">${user} <span style="font-size:13px; color:#10b981; font-weight:700;">${counts[user]} Clients</span></h4><div style="width: 100%; background: var(--border-input); height: 8px; border-radius: 4px; overflow: hidden;"><div style="background: linear-gradient(90deg, #3b82f6, #10b981); height: 100%; border-radius: 4px; width: ${pct}%"></div></div></div></div>`; } document.getElementById('teamActivityContainer').innerHTML = html || "<p>No activity found.</p>"; }
...
(function initPwaInstall() {
    ...
    const popupElement = document.getElementById('pwaInstallPopup');
    const installBtn = document.getElementById('pwaInstallBtn');

    function checkAndShowPopup() {
        const isInstalled = window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone;
        const isMobileDevice = /Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(navigator.userAgent) || window.innerWidth <= 992;

        if (!isInstalled && isMobileDevice && installPrompt && popupElement) {
